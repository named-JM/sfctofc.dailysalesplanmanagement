...
public class ExcelService : IExcelService
{
    ...
    /// <returns></returns>
    public async Task<string> CopyFileAsync(string sharedPath, string localPath)
    {
        ...
        using var sourceStream = File.Open(sharedPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
    ...
    public Task<List<string[]?>> ReadCSVFileLogsAsync(string localFilePath)
    {
        ...
        //{
        //    var _machine = row.Cell(1).GetValue<string>();
        //    var _batch = row.Cell(2).GetValue<int>();
        //    var _processDate = row.Cell(3).GetValue<DateTime>();
        //    machineLogs.Add(new ProductionTicketMachineLogs { Machine = _machine, Batch = _batch, ProcessDate = _processDate });
        //}

        var partsList = new List<string[]?>();

        foreach (var line in File.ReadLines(localFilePath).Skip(1)) // skip header
        {
            if (string.IsNullOrWhiteSpace(line)) continue;

            var parts = line.Split(','); // split CSV row into columns

            partsList.Add(parts);
        }

        return Task.FromResult(partsList);
    }
}
